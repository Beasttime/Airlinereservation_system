...
public class InternationalFlight extends JFrame
{
	...
	public InternationalFlight(LoginPage type1)
	{
		...
		CBFrom = extracted(sItem1);
		CBTo = extracted(sItem2);
		CBClass = extracted(sItem3);
		...
		CBAdult = extracted(item4);

		String[] item5={"0","1","2","3","4"};
		CBChildren = extracted(item5);

		String[] item6={"0","1","2","3"};
		CBInfant = extracted(item6);
	...
	private JComboBox extracted(String[] item4) {
		return new JComboBox(item4);
	}
...
//write into data
				...
				try (ObjectOutputStream OOS1 = new ObjectOutputStream(new FileOutputStream("save1"))) {
					for(i=0;i<iCount;i++)
					{
